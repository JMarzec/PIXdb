...
if ( length(absentGenes) > 0 ) {
    ...
    write(absentGenes, file = "absent_genes.txt", append = FALSE, sep="\t")
}
...
##### Keep 2e00 genes with the highest expression variance across samples
...
#pdf("heatmap.pdf", width = 10, height = ncol(expData)/5, pointsize = 12)
...
htmlwidgets::saveWidget(as_widget(p), "heatmap_exp_1.html", selfcontained = FALSE)
